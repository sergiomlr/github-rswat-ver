...
#Statistics 
sens_fast <- sensitivity(r2_fast, 17) # These are the number of parameters
...
```{r}
pwn <- tempawn(sim = q_fast)

#Visualization tempPAWN for FDC
fdc_sim <- pwn$simulation$q_out%>%
...
# Visualization of FDC parameters
fdc_sens<-fdc_sim%>%
  left_join(., pwn$sensitivity$q_out)%>%
...
pwn_mon <- pwn$sensitivity$q_out%>%
  mutate(month = format(date, "%m"))%>%
  group_by(month)%>%
  #group_by(month = floor_date(date, "month"))%>%
  summarize_all(., ~mean(.))%>%
  pivot_longer(., cols = colnames(pwn_mon[3:20]), names_to ="parameter") # Create a long table
