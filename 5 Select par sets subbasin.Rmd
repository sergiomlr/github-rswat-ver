...
print(order(gav$rsr_Q95, decreasing=F)[1:300])  # The highest 10 percentile of simulations in terms of RSR_Q95

print(order(gav$kge, decreasing=T)[1:300])  # The highest 10 percentile of simulations in terms of KGE


print(intersect(order(gav$rsr_Q95, decreasing=F)[1:750],order(gav$kge, decreasing=T)[1:750]))

#fdc  
bestpar$simsgav <- data.frame(gav$sim_pwn[c(intersect(order(gav$rsr_Q95, decreasing=F)[1:750],order(gav$kge, decreasing=T)[1:750]))+1])%>%
  lapply(., sort, decreasing=T)%>%
  data.frame(.)%>%
  mutate(p = seq(0,1, length.out = nrow(.)))%>%
  melt(.,  id.vars = 'p', variable.name = 'series') #To long dataframe
